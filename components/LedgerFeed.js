...
//
// groupCardsByType is imported rather than defined here: both server pages
// call it, and an export from a 'use client' module becomes a client
// reference that throws when invoked during a server render.
...
export default function LedgerFeed({ cards, savedCardIds, userId, edition, dateLabel, isArchive = false }) {
  ...
  return (
            totalCards === 0 ? <EmptyState /> : (
              <>
                ...
                <div id="section-scripture">
                  {grouped.scripture?.map((card) => (
                    <FadeIn key={card.id} delay={0}>
                      <ScriptureCard card={card} savedCardIds={savedCardIds} userId={userId} />
                    </FadeIn>
                  ))}
                </div>
                ...
                <div id="section-quote">
                  {grouped.quote?.map((card) => (
                    <FadeIn key={card.id} delay={100}>
                      <QuoteCard card={card} savedCardIds={savedCardIds} userId={userId} />
                    </FadeIn>
                  ))}
                </div>
                    ...
                    <div className="tdl-qf-grid">
                      {grouped.quick_facts.map((card, i) => (
                        <FadeIn key={card.id} delay={i * 60}>
                          <QuickFactCard card={card} savedCardIds={savedCardIds} userId={userId} />
                        </FadeIn>
                      ))}
                    </div>
                  </div>
                  ...
                  <div id="section-book_summary" className="tdl-section">
                    ...
                    {grouped.book_summary.map((card, i) => (
                      <FadeIn key={card.id} delay={i * 80}>
                        <BookSummaryCard card={card} savedCardIds={savedCardIds} userId={userId} />
                      </FadeIn>
                    ))}
                  </div>
                )}
                ...
                {grouped.food_spotlight?.length > 0 && (
                  <div id="section-food_spotlight" className="tdl-section">
                    ...
                    {grouped.food_spotlight.map((card) => (
                      <FadeIn key={card.id}>
                        <FoodSpotlightCard card={card} savedCardIds={savedCardIds} userId={userId} />
                      </FadeIn>
                    ))}
                  </div>
                )}
                ...
                {grouped.research?.length > 0 && (
                  <div id="section-research" className="tdl-section">
                    ...
                    {grouped.research.map((card, i) => (
                      <FadeIn key={card.id} delay={i * 80}>
                        <ResearchCard card={card} savedCardIds={savedCardIds} userId={userId} />
                      </FadeIn>
                    ))}
                  </div>
                )}
                ...
                {grouped.protocol?.length > 0 && (
                  <div id="section-protocol" className="tdl-section">
                    ...
                    {grouped.protocol.map((card) => (
                      <FadeIn key={card.id}>
                        <ProtocolCard card={card} savedCardIds={savedCardIds} userId={userId} />
                      </FadeIn>
                    ))}
                  </div>
                )}
                ...
                <FadeIn>
                  <EndOfLedger cards={cards} edition={edition} dateLabel={dateLabel} isArchive={isArchive} />
                </FadeIn>
              </>
            )
  )
